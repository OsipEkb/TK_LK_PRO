...
const VehicleCard = ({ vehicle, online = {}, theme }) => {
  // Расчет статуса на основе DT из API
  const getStatus = (dt) => {
  ...
  const status = getStatus(online.DT || online.LastData);
  const fuelPerc = vehicle.max_fuel > 0
    ? Math.min(100, Math.round(((online.fuel || online.Fuel || 0) / vehicle.max_fuel) * 100))
    : 0;
    ...
    <div className="p-6 border flex flex-col justify-between h-[420px] relative transition-all shadow-xl group"
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="text-[10px] font-bold opacity-40 uppercase text-white">
            {vehicle.reg_number || vehicle.RegNumber || 'Без номера'}
          </div>
        </div>
        <div className="text-right">
          <div className="text-3xl font-black italic" style={{ color: theme.accent }}>
            {Math.round(online.Speed || online.speed || 0)}
          </div>
      ...
      <div className="bg-black/20 p-4 rounded-2xl border border-white/5 flex gap-3 items-start my-4">
        ...
        <p className="text-[11px] leading-tight opacity-70 text-white line-clamp-2">
          {online.Address || online.address || 'Местоположение не определено'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
          ...
          <div className="font-black text-white">
            {online.moto_hours || online.Moto || online.engine_hours || 0} ч
          </div>
        </div>
        <div className="bg-white/5 p-3 rounded-xl border border-white/5">
          <span className="text-[8px] font-black opacity-30 uppercase block mb-1">Статус</span>
          <div className="font-black text-[10px]" style={{ color: status.color }}>
            {status.label}
          </div>
        </div>
      </div>
      ...
      <div className="p-5 rounded-[24px] text-black shadow-inner relative overflow-hidden"
        ...
        <div className="flex justify-between items-end mb-1 relative z-10">
          <span className="uppercase text-[9px] font-black flex items-center gap-1">
            <Fuel size={12}/> Топливо {fuelPerc}%
          </span>
          <span className="text-xl font-black italic">
            {Math.round(online.fuel || online.Fuel || 0)} Л
          </span>
        </div>
        <div className="w-full h-1.5 bg-black/10 rounded-full mt-2 overflow-hidden relative z-10">
          <div
            ...
            style={{ width: `${fuelPerc}%` }}
          />
        </div>
      </div>

      <div className={`mt-4 text-[9px] font-black uppercase flex items-center gap-2 ${
        online.ignition || online.Ignition ? 'text-green-500' : 'opacity-20 text-white'
      }`}>
        <Zap size={12} fill={(online.ignition || online.Ignition) ? "currentColor" : "none"} />
        {(online.ignition || online.Ignition) ? 'Зажигание включено' : 'Зажигание выключено'}
      </div>
    </div>
  );
};
